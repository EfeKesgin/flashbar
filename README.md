...
Flashbar is engineered to be extremely lightweight and fast, running with minimal memory and CPU footprint:

- **RAM Usage**: ~90 MB to 120 MB maximum (WebView2 engine included).
- **CPU Usage**: 0% on idle, maximum 2% during active searching and filtering.
- **Benchmark Environment**: Tested on Windows 11 with AMD Ryzen 5 5600.
...
## Pre-requisites & Security Configuration

To ensure a smooth user experience, the setup installer automatically performs the following system configurations:
...
2. **Startup Configuration**: Configures a user-space registry key under `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` to launch the application automatically on user logon, running with standard user privileges.
...
2. Run the installer (requires administrator privileges to register the code-signing certificate for SmartScreen bypass).
...
### Search Bar Overlay
![Flashbar Search Bar](https://raw.githubusercontent.com/EfeKesgin/flashbar/refs/heads/assets/searchbar.png)

### Settings Panel
![Flashbar Settings](https://raw.githubusercontent.com/EfeKesgin/flashbar/refs/heads/assets/settings.png)
